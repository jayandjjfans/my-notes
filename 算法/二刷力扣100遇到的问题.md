...
主要是记忆相关的API，构造单调递减队列，并且最好使用一个for循环解决
...
这道题是可以套用常规的滑动窗口模版的，但是有些细节是需要注意的，这次做题被卡了一下，需要常做。

1. 本题需要构造两个int[128]数组，同字母异位词不同（需要完全一致，可以做到用一个数组区分两个字符串的区别），本题是覆盖关系，对于大串中多余的字符，不用和小串做任何比较。
2. 当找到符合条件的窗口后，在循环缩小窗口的过程中，判断diff是否需要增加，使用ss[s.charAt(left)]==tt[s.charAt(left)]。这次做题在这块写的不够简洁。

##### 柱状图中最大的矩形 

此题之前做过一遍，若记得思路其实逻辑并不难，细节也不多。一定要常复习并捋清思路。

##### 寻找两个正序数组的中位数

此次第一次做，并整理了代码逻辑。难度是HOT100中最高，需要经常复习！

待整理API

LinkedList、PriorityQueue、Stack、ArrayDeque

待总结

二分查找mid向上取整和向下取整的区别；循环条件 left<right和left<=right的区别
