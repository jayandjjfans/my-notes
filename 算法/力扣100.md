        ...
        for (int i = 1; i < len; i++) {
            ...
            if (s.charAt(i) == r) {
                ...
                if (s.charAt(i - 1) == l) {
                    ...
                    //只有前一括号结尾的右括号为有效，且当前位置括号能组成额外有效括号
                    if (dp[i - 1] != 0 && i - 1 - dp[i - 1] >= 0 && s.charAt(i - 1 - dp[i - 1]) == l) {
                        dp[i] = dp[i - 1] + 2;
                        //如果此括号结尾的有效括号的前面还可能存在有效括号
                        if (i - 2 - dp[i - 1] >= 0) {
...
class Solution {
    public int findMin(int[] nums) {
        ...
        while (l < r) {
            int mid = (l + r) / 2;
            //升序曲线图情况一共有三种，1左长右短、2左短右长、3左右相等、4全局升序
...
用单调递减队列（LinkedList）维护数组元素索引。第一次循环形成窗口，第二次循环滑动窗口，注意都要维护递减的规则然后添加到返回值里。而且滑动窗口时需要判断当前队首（最大值）是否超出窗口范围并且移除。
